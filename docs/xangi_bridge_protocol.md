...
| `clock` | string | 最後に受け取った `HH:MM`。未受信なら空文字 | 0.20 |
| `clock_visible` | bool | `clock` を左上overlayとして表示中か。90秒以上更新が無いと false | 0.20 |
| `clock_age_ms` | int | 最後の `TIME:` 受信からの経過ms。未受信なら 0 | 0.20 |
...
PC 側 bridge が現在時刻を `HH:MM` 形式で送る。ファームは値を検証して保持し、
Avatar 顔 / 画像顔の左上に黒背景 + 白文字の小さな時計overlayを描く。

成功時:

```json
{"status":"ok","time":"09:42"}
```

不正な時刻 (`9:42` / `24:00` / `12:60` / `abc` 等) は保存せず、次を返す:

```json
{"status":"error","error":"TIME syntax: HH:MM"}
```
...
| 2026-06-20 | 左上時計overlayを追加。host から `TIME:HH:MM` を毎分送り、firmware は Avatar 顔 / 画像顔の左上に黒背景 + 白文字で時刻を描画する。90秒更新が無ければ非表示。STATUS に `clock` / `clock_visible` / `clock_age_ms` を追加。version `cores3-main-0.20` |
| 2026-06-20 | Avatar 顔の時計overlayを M5Stack-Avatar のスプライト描画hook内で描くように変更。`M5.Display` への直接描画と Avatar 描画タスクの SPI transaction 競合による panic 再起動を避ける。version `cores3-main-0.21` |
