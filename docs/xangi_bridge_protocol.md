...
| コマンド | 引数 | 応答 | 状態 |
|---------|------|------|------|
| `STATUS` | なし | `{"state","volume","version","servo","torque","queued","playing","image_face","battery_level","battery_voltage_mv","charging"}` JSON | WAV シリアル受信実装 で初版、サーボ統合 PR で `servo`/`torque`、WAV キュー化 PR で `queued`/`playing` 追加、画像顔/バッテリー表示 PR で `image_face`/battery 追加 |
